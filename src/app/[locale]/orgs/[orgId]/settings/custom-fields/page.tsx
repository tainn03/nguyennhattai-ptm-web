...
import { CUSTOM_FIELD_TYPE } from "@/constants/custom-fields";
import { SESSION_FLASHING_ID } from "@/constants/storage";
import { useCustomFields, useIdParam, usePermission, useSearchConditions } from "@/hooks";
...
export default withOrg(
  ({ orgId, orgLink, userId }) => {
    ...
    const { searchConditions } = useCustomFieldState();
    ...
    const { canNew, canEdit, canEditOwn, canDelete, canDeleteOwn } = usePermission("custom-field");

    const updateRouteRef = useRef(false);
    const selectedCustomFieldRef = useRef<CustomFieldInfo>();
    const { isLoading, customFields, pagination, mutate } = useCustomFields({
    ...
    useEffect(() => {
      setBreadcrumb([
        ...
        { name: t("custom_field.title"), link: `${orgLink}/settings/custom-fields` },
      ]);
    ...
    useEffect(() => {
      if (updateRouteRef.current) {
        ...
        dispatch<FilterOptions>({
          type: CUSTOM_FIELD_UPDATE_SEARCH_CONDITIONS,
          payload: filterOptions,
        });
        dispatch<string>({
          type: CUSTOM_FIELD_UPDATE_SEARCH_QUERY_STRING,
          payload: queryString,
        });
      }
      // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    /**
     * Callback function for opening a dialog with custom field data.
     *
     * @param item - The custom field data to display in the dialog.
     */
    const handleDelete = useCallback(
      (item: CustomFieldInfo) => () => {
        selectedCustomFieldRef.current = item;
    ...
    const handleDeleteConfirm = useCallback(async () => {
      if (selectedCustomFieldRef.current?.id && userId) {
        const { error } = await deleteCustomField(
          {
            organizationId: orgId,
            id: Number(selectedCustomFieldRef.current.id),
            updatedById: userId,
          },
          selectedCustomFieldRef.current.updatedAt
        );
        ...
        if (error) {
          showNotification({
            ...
            message: t("common.message.delete_error_message", {
              name: selectedCustomFieldRef.current?.name,
            }),
          });
        } else {
          showNotification({
    ...
    const handleFilterApply = useCallback(
      (columnName: string) => (filters: FilterProperty[], sortType?: SortType) => {
        ...
        setFilterOptions((prevValue) => {
          ...
          // Auto add sort column display order if column type is checked
          if (columnName === "type") {
            const isExistTypeChecked = (newValue["type"].filters[0].value as CheckboxItem[]).some(
              (item) => item.checked
            );
            if (isExistTypeChecked) {
              newValue["displayOrder"].sortType = "asc";
            }
          }
          return newValue;
        });
    ...
    return (
      <>
        <PageHeader
          ...
          actionComponent={
            <Authorization resource="custom-field" action="new">
              <Button as={Link} href={`${orgLink}/settings/custom-fields/new`} icon={PlusIcon}>
                {t("common.new")}
              </Button>
            </Authorization>
          }
              ...
              <TableRow>
                ...
                <TableCell>
                  <TableFilterMenu
                    ...
                    actionPlacement="right"
                    {...filterOptions.displayOrder}
                    onApply={handleFilterApply("displayOrder")}
                  />
                </TableCell>
                <TableCell>
                  <TableFilterMenu
                ...
                <TableCell>
                  <TableFilterMenu
                    ...
                    onApply={handleFilterApply("dataType")}
                  />
                </TableCell>
            ...
            <TableBody>
              ...
              {!isLoading && customFields.length === 0 && (
                <TableRow hover={false} className="mx-auto max-w-lg">
                  <TableCell colSpan={9} className="px-6 lg:px-8">
                    <EmptyListSection actionLink={`${orgLink}/settings/custom-fields/new`} />
                  </TableCell>
                </TableRow>
              )}
              ...
              {customFields.map((item, index) => (
                <TableRow key={item.id} flash={Number(item.id) === flashingId} onFlashed={handleFlashed}>
                  <TableCell>{customFieldTypeOptions.find((data) => data.value === item.type)?.label}</TableCell>
                  <TableCell>{item.displayOrder ?? "-"}</TableCell>
                  <TableCell className="max-w-[268px] [&>a]:!whitespace-pre-wrap [&>a]:break-all">
                    <Authorization
                      resource="custom-field"
                      action="detail"
                      fallbackComponent={
                        <span className="!whitespace-pre-wrap break-all text-sm font-medium leading-6 text-gray-900">
                          {item.name}
                        </span>
                      }
                    >
                      <Link
                        ...
                        href={`${orgLink}/settings/custom-fields/${encryptId(item.id)}`}
                      >
                        {item.name}
                      </Link>
                    </Authorization>
                  </TableCell>
                  <TableCell className="max-w-[268px] !whitespace-pre-wrap break-all">{item.key}</TableCell>
                  <TableCell>{item.dataType}</TableCell>
                  <TableCell align="center">
                    <Badge
                      label={item.isActive ? t("custom_field.status_active") : t("custom_field.status_inactive")}
                    ...
                    >
                      <MasterActionTable
                        actionPlacement={
                          ...
                          (customFields.length - 1 === index || customFields.length - 2 === index)
                            ? "start"
                            : "end"
                        }
                        editLink={
                          canEdit() || (canEditOwn() && equalId(item.createdByUser.id, userId))
                            ? `${orgLink}/settings/custom-fields/${encryptId(item.id)}/edit`
                            : ""
                        }
                        copyLink={canNew() ? `${orgLink}/settings/custom-fields/new?copyId=${encryptId(item.id)}` : ""}
                        onDelete={
        ...
        <ConfirmModal
          ...
          icon="error"
          color="error"
          title={t("common.confirmation.delete_title", { name: selectedCustomFieldRef.current?.name })}
