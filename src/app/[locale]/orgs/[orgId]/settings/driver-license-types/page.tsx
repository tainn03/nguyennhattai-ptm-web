...
import { useDriverLicenseTypes, useIdParam, usePermission, useSearchConditions } from "@/hooks";
...
export default withOrg(
  ({ orgId, orgLink, userId }) => {
    ...
    const { searchConditions } = useDriverLicenseTypeState();
    ...
    const { canNew, canEdit, canEditOwn, canDelete, canDeleteOwn } = usePermission("driver-license-type");

    const updateRouteRef = useRef(false);
    const selectedMerchandiseTypeRef = useRef<DriverLicenseTypeInfo>();

    const { isLoading, driverLicenseTypes, pagination, mutate } = useDriverLicenseTypes({
    ...
    useEffect(() => {
      setBreadcrumb([
        ...
        { name: t("driver_license_type.title"), link: `${orgLink}/settings/driver-license-types` },
      ]);
    ...
    useEffect(() => {
      if (updateRouteRef.current) {
        ...
        dispatch<FilterOptions>({
          type: DRIVER_LICENSE_TYPE_UPDATE_SEARCH_CONDITIONS,
          payload: filterOptions,
        });
        dispatch<string>({
          type: DRIVER_LICENSE_TYPE_UPDATE_SEARCH_QUERY_STRING,
          payload: queryString,
        });
      }
      // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    /**
     * Callback function for opening a dialog with driver license type data.
     *
     * @param item - The driver license type data to display in the dialog.
     */
    const handleDelete = useCallback(
      (item: DriverLicenseTypeInfo) => () => {
    ...
    const handleDeleteConfirm = useCallback(async () => {
      ...
      handleDeleteCancel();
      mutate();
    }, [handleDeleteCancel, mutate, orgId, showNotification, userId, t]);
      ...
      <>
        <PageHeader
          ...
          actionComponent={
            <Authorization resource="driver-license-type" action="new">
              <Button as={Link} href={`${orgLink}/settings/driver-license-types/new`} icon={PlusIcon}>
                {t("common.new")}
              </Button>
            </Authorization>
          }
        ...
        <TableContainer fullHeight>
          <Table>
            ...
            <TableBody>
              {/* Loading skeleton */}
              {isLoading && driverLicenseTypes.length === 0 && (
              ...
              {!isLoading && driverLicenseTypes.length === 0 && (
                <TableRow hover={false} className="mx-auto max-w-lg">
                  <TableCell colSpan={5} className="px-6 lg:px-8">
                    <EmptyListSection actionLink={`${orgLink}/settings/driver-license-types/new`} />
                  </TableCell>
                </TableRow>
              )}

              {/* Data */}
              {driverLicenseTypes.map((item, index) => (
                <TableRow key={item.id} flash={Number(item.id) === flashingId} onFlashed={handleFlashed}>
                  <TableCell>
                    ...
                    >
                      <Link
                        ...
                        href={`${orgLink}/settings/driver-license-types/${encryptId(item.id)}`}
                      >
                        {item.name}
                      </Link>
                    </Authorization>
                  ...
                  <TableCell action>
                    ...
                    >
                      <MasterActionTable
                        actionPlacement={
                          driverLicenseTypes.length >= 3 &&
                          (driverLicenseTypes.length - 1 === index || driverLicenseTypes.length - 2 === index)
                            ? "start"
                            : "end"
                        }
                        editLink={
                          canEdit() || (canEditOwn() && equalId(item.createdByUser.id, userId))
                            ? `${orgLink}/settings/driver-license-types/${encryptId(item.id)}/edit`
                            : ""
                        }
                        copyLink={
                          canNew() ? `${orgLink}/settings/driver-license-types/new?copyId=${encryptId(item.id)}` : ""
                        }
                        onDelete={
