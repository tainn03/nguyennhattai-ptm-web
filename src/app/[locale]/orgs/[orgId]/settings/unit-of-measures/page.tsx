...
import { useIdParam, usePermission, useSearchConditions, useUnitOfMeasures } from "@/hooks";
...
export default withOrg(
  ({ orgId, orgLink, userId }) => {
    ...
    const { searchConditions } = useUnitOfMeasureState();
    ...
    const { canNew, canEdit, canEditOwn, canDelete, canDeleteOwn } = usePermission("unit-of-measure");

    const updateRouteRef = useRef(false);
    const selectedMerchandiseTypeRef = useRef<UnitOfMeasureInfo>();

    const { isLoading, unitOfMeasures, pagination, mutate } = useUnitOfMeasures({
    ...
    useEffect(() => {
      setBreadcrumb([
        ...
        { name: t("unit_of_measure.title"), link: `${orgLink}/settings/unit-of-measures` },
      ]);
    ...
    useEffect(() => {
      if (updateRouteRef.current) {
        ...
        dispatch<FilterOptions>({
          type: UNIT_OF_MEASURE_UPDATE_SEARCH_CONDITIONS,
          payload: filterOptions,
        });
        dispatch<string>({
          type: UNIT_OF_MEASURE_UPDATE_SEARCH_QUERY_STRING,
          payload: queryString,
        });
      }
      // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const handleDeleteConfirm = useCallback(async () => {
      if (selectedMerchandiseTypeRef.current?.isSystem) {
        return;
      }
      ...
      <>
        <PageHeader
          ...
          actionComponent={
            <Authorization resource="unit-of-measure" action="new">
              <Button as={Link} href={`${orgLink}/settings/unit-of-measures/new`} icon={PlusIcon}>
                {t("common.new")}
              </Button>
            </Authorization>
          }
        ...
        <TableContainer fullHeight>
          <Table>
            ...
            <TableBody>
              {/* Loading skeleton */}
              {isLoading && unitOfMeasures.length === 0 && (
              ...
              {!isLoading && unitOfMeasures.length === 0 && (
                <TableRow hover={false} className="mx-auto max-w-lg">
                  <TableCell colSpan={5} className="px-6 lg:px-8">
                    <EmptyListSection actionLink={`${orgLink}/settings/unit-of-measures/new`} />
                  </TableCell>
                </TableRow>
              )}
              ...
              {unitOfMeasures.map((item, index) => (
                <TableRow key={item.id} flash={Number(item.id) === flashingId} onFlashed={handleFlashed}>
                  ...
                  <TableCell>
                    ...
                    >
                      <Link
                        ...
                        href={`${orgLink}/settings/unit-of-measures/${encryptId(item.id)}`}
                      >
                        {item.name}
                      </Link>
                    </Authorization>
                  ...
                  <TableCell align="center">
                    <Badge
                      label={item.isActive ? t("unit_of_measure.status_active") : t("unit_of_measure.status_inactive")}
                  ...
                  <TableCell action>
                    ...
                    >
                      <MasterActionTable
                        actionPlacement={
                          unitOfMeasures.length >= 3 &&
                          (unitOfMeasures.length - 1 === index || unitOfMeasures.length - 2 === index)
                            ? "start"
                            : "end"
                        }
                        editLink={
                          canEdit() || (canEditOwn() && equalId(item.createdByUser.id, userId))
                            ? `${orgLink}/settings/unit-of-measures/${encryptId(item.id)}/edit`
                            : ""
                        }
                        copyLink={
                          canNew() ? `${orgLink}/settings/unit-of-measures/new?copyId=${encryptId(item.id)}` : ""
                        }
                        onDelete={
