...
import { notifyAndUpdateStatusInStock } from "@/actions/orderGroup";
...
export const POST = withExceptionHandler(async (request: ApiNextRequest, requestData: WarehouseNotifyRequest) => {
  ...
  // Process the in-stock notification and status update
  const result = await notifyAndUpdateStatusInStock(requestData);

  // Return success response with updated data
  return {
    status: HttpStatusCode.Ok,
    data: result,
    message: "In-stock orders notified and status updated",
  };
});
