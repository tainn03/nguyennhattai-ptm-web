...
import useNotification from "@/redux/actions/useNotification";
...
export type OrderShareModalProps = {
  open: boolean;
  order?: Partial<OrderInfo> | null;
  onClose: () => void;
  onCancel: () => void;
};

const OrderShareModal = ({ open, order, onClose, onCancel }: OrderShareModalProps) => {
  ...
  const orderId = open && order?.id ? Number(order.id) : undefined;
  const { shareObject, isLoading, mutate } = useShareObject({ id: orderId });
  ...
  /**
   * Callback function to handle stopping the sharing of an order.
   */
  ...
  /**
   * handleConfirm is a callback function that generates an order share token and updates the UI.
   *
   * @remarks
  ...
  const handleConfirm = useCallback(async () => {
    if (!orgId || !userId || !orderId || !order?.code) {
      return;
    }
  ...
  }, [orgId, userId, orderId, order?.code, expireDate, t, isShareMap]);
  ...
  const handleStopSharing = useCallback(async () => {
    if (!order || !shareObject || !userId) return;
    setIsShareObjectCreating(true);
    try {
      setOpenConfirm(false);
  ...
  }, [order, shareObject, userId, showNotification, t, mutate]);
  ...
  return (
    <Modal open={open} divider={false} className="sm:!max-w-lg" onDismiss={onClose} allowOverflow>
            ...
            <>
              <div className="mt-2">
                <p className="text-sm tracking-tight text-gray-500">
                  {t.rich("order.share_modal.message.order_code", {
                    strong: (chunks) => <strong>{chunks}</strong>,
                    code: order?.code,
                  })}
                </p>
      ...
      <ModalActions className="pb-4 pt-0 sm:pb-6">
        ...
        {shareObject && (
          <Authorization resource="order" action="cancel-share">
            <Button
              color="error"
              className="min-w-[120px] flex-1"
        ...
        {shareObject && (
          <ConfirmModal
            ...
            message={t("order.share_modal.message.confirm_stop_sharing")}
