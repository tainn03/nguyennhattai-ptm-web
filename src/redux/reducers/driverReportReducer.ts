...
const initialState: DriverReportState = {
  searchConditions: {
    pagination: {
      page: 1,
...
const driverReportReducer: Reducer<DriverReportState, Action> = (state = initialState, action) => {
  switch (action.type) {
    case DRIVER_REPORT_UPDATE_SEARCH_CONDITIONS:
      return {
        ...state,
        searchConditions: { ...action.payload },
      };

    case DRIVER_REPORT_UPDATE_SEARCH_QUERY_STRING:
      return {
        ...state,
...
export default driverReportReducer;
