...
import { TRAILER_TYPE_UPDATE_SEARCH_CONDITIONS, TRAILER_TYPE_UPDATE_SEARCH_QUERY_STRING } from "@/redux/types";
import { FilterOptions } from "@/types/filter";

type TrailerTypeState = {
...
const initialState: TrailerTypeState = {
  searchConditions: {
    ...
    name: {
      sortLabel: "trailer_type.name",
      filters: [
        {
          filterLabel: "trailer_type.name",
          name: "name",
          type: "text",
          placeholder: "trailer_type.name_placeholder",
          value: "",
        },
      ],
    },
    isActive: {
      sortLabel: "trailer_type.status",
      filters: [
        {
          filterLabel: "trailer_type.status",
          name: "isActiveOptions",
          type: "checkbox",
          value: [
            { value: "true", label: "trailer_type.status_active", checked: true },
            { value: "false", label: "trailer_type.status_inactive" },
          ],
        },
      ],
    },
...
const trailerTypeReducer: Reducer<TrailerTypeState, Action> = (state = initialState, action) => {
  switch (action.type) {
    case TRAILER_TYPE_UPDATE_SEARCH_CONDITIONS:
      return {
        ...state,
        searchConditions: { ...action.payload },
      };

    case TRAILER_TYPE_UPDATE_SEARCH_QUERY_STRING:
      return {
        ...state,
...
export default trailerTypeReducer;
